...
- Tải lên bức ảnh chụp món ăn thuộc 50 nhóm nhưng ở góc nghiêng lạ hoặc đang chan nước dùng khiến mô hình chính ONNX có độ tin cậy thấp (55.3% < 60%).
- Kết quả kỳ vọng: Độ tin cậy ONNX < 60% ➔ Lambda tự động kích hoạt Amazon Rekognition (detect_labels) quét nhãn tổng quan (Soup, Noodle, Broth...) ➔ Ánh xạ thành công sang món Phở Bò (380 KCAL) và ghi log ảnh vào s3://.../ood_logs/.
- Thông số phản hồi: Giao diện Web hiển thị Engine: Amazon Rekognition fallback và độ tin cậy AI: 55.3%.
...
- Tải lên ảnh món ăn không nằm trong 50 món đã học và không có từ khóa Rekognition nào khớp (ví dụ: món bánh phô mai que hoặc món ăn địa phương hiếm gặp).
- Kết quả kỳ vọng: Cả ONNX lẫn Rekognition đều không khớp được ➔ Hệ thống chủ động trả về HTTP 422 thay vì đoán sai. Ảnh OOD được lưu tự động vào s3://.../ood_logs/ để xem xét tái huấn luyện sau.
